...
export function calcularRiesgoRuta(origen: { alcaldia: string, colonia: string }, destino: { alcaldia: string, colonia: string }, zonasRiesgo: ZonaRiesgo[]): {
  score: number
  nivel: 'bajo' | 'medio' | 'alto' | 'critico'
  recomendacion: string
} {
  const zOrigen = zonasRiesgo.find(z => z.alcaldia === origen.alcaldia && z.colonia === origen.colonia)
  const zDestino = zonasRiesgo.find(z => z.alcaldia === destino.alcaldia && z.colonia === destino.colonia)

  const sOrigen = zOrigen?.severidad || 3
  const sDestino = zDestino?.severidad || 3
  
  const score = (sOrigen + sDestino) / 2
  
  let nivel: 'bajo' | 'medio' | 'alto' | 'critico' = 'bajo'
  let recomendacion = 'Ruta relativamente segura. Mantenga precaución estándar.'

  if (score >= 8) {
    nivel = 'critico'
    recomendacion = '¡Riesgo Muy Alto! Se recomienda buscar rutas alternas o evitar esta zona en horarios de alta afluencia.'
  } else if (score >= 6) {
    nivel = 'alto'
    recomendacion = 'Riesgo Elevado. Conduzca con extrema precaución y verifique el estado de frenos.'
  } else if (score >= 4) {
    nivel = 'medio'
    recomendacion = 'Riesgo Moderado. Esté atento a cruces e intersecciones peligrosas.'
  }

  return { score, nivel, recomendacion }
}
